...
const FilterButtons = ({filterData, flag}) => {
    ...
    const updateFilterButton = (name) => {
        ...
        if(flag === "Artist"){
            dispatch({
                type: actionType.SET_ARTIST_FILTER, artistFilter: name,
            })
        }

        if(flag === "Album"){
            dispatch({
                type: actionType.SET_ALBUM_FILTER, albumFilter: name,
            })
        }

        if(flag === "Language"){
            dispatch({
                type: actionType.SET_LANGUAGE_FILTER, languageFilter: name,
            })
        }

        if(flag === "Category"){
            dispatch({
                type: actionType.SET_FILTER_TERM, filterTerm: name,
            })
        }
    };
  return (
    <div className='border border-gray-300 rounded-md px-4 py-1 relative cursor-pointer hover:border-gray-400'>
        ...
        </p>
        {
            filterData && filterMenu && (
                ...
                exit={{opacity: 0, y: 50}}
                className="w-35  z-50 backdrop-blur-sm max-h-44 overflow-y-scroll scrollbar-thin scrollbar-track-gray-200 scrollbar-thumb-gray-400 py-2 flex flex-col rounded-md shadow-md absolute top-8 left-0 ">{
                    filterData?.map(data => (
