...
const MusicPlayer = () => {
  ...
  return (
    <div className='w-full flex flex-col items-center gap-3 overflow-hidden'>
        <div className={`w-full items-center gap-3 p-4 ${
          miniPlayer ? "hidden" : "flex relative"
        }`}>
            <img src={allSongs[song]?.imageURL} alt="" className='w-40 h-20 object-cover rounded-md'/> 
            <div className='flex items-start flex-col'>
                ...
                <p className='text-textColor'>
                    ...
                    <span className='text-sm text-textColor font-semibold'>({allSongs[song]?.category})</span>
                </p>

                <motion.i
                whileTap={{scale: 0.8}} onClick={() => setIsPlayList(!isPlayList)}>
        ...
        <div className='absolute left-4 bottom-24 gap-2 py-2 w-350 max-w-[350px] h-510 max-h-[510px] flex flex-col overflow-y-scroll scrollbar-thin rounded-md shadow-md bg-primary'>
            {allSongs.length > 0 ? (
                allSongs.map((music, index) => (
                    ...
                    transition={{duration: 0.3, delay: index * 0.1}} className='group w-full p-4 hover:bg-card flex gap-3 items-center cursor-pointer bg-transparent' onClick={() => setCurrentPlaySong(index)}
                    key={index}>
