...
router.get("/login", async(req, res) => {
    ...
    try{
        ...
        console.log("dec:",decodeValue);
        if(!decodeValue) {
            return res.status(505).json({message : "Un Authorized"})
        }else {
           //checking user exists or not a
        ...
        return res.status(505).json({message: error.message}); // Corrected to send error.message
    }
})
...
router.put("/updateRole/:userId/:role", async (req, res) => {
    const filter = {_id: req.params.userId};
    const role = {r: req.params.role};
    try{
        const result = await user.findOneAndUpdate(filter, {role: role.r});
        res.status(200).send({user: result});
    }catch(error) {
        res.status(400).send({success: false, msg: error});
    }
